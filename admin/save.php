	...
	switch ($action) {
		...
		case 'SaveEmployment':
			...
			if($position){
				...
				elseif($curr_position_status == 1){
					//---Handle error
					$json_data = '{"result":"position_filled"}'; 
				}
				//---If current position is unfunded = 2
				elseif($curr_position_status == 2){
					//---Handle error
					$json_data = '{"result":"position_unfunded"}'; 
				}
				else{ 
					$json_data = '{"result":"xxx"}'; 
				}
		...
		case 'delete_employment':
			...
			//---After deleting employment change the status of the position to vacant---//
			if ($MyEmployment->DeleteEmployment($employment_id) && $MyPosition->UpdatePositionStatus($position_id,'0')) {
				$json_data = '{"result":"success"}';
			}
			else{
				$json_data = '{"result":"xxx"}';
